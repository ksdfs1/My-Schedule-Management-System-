...
public class ScheduleManager {  //ScheduleManager 클래스
	...
	private ArrayList<ScheduleInput> scheduleList = new ArrayList<ScheduleInput>();  //schedule 객체의 주소를 저장하는 리스트
	...
	public void addScheduleList(ScheduleInput scheduleInput) {  //리스트에 scheduleInput 객체의 주소를 추가하는 함수
		scheduleList.add(scheduleInput);
	}
	
	public void deleteScheduleList(ScheduleInput scheduleInput) {  //리스트에 scheduleInput 객체의 주소를 삭제하는 함수
		scheduleList.remove(scheduleInput);
	}
	
	public void searchScheduleList(String content) {  //리스트에 있는 scheduleInput 객체의 주소 중 원하는 주소를 찾는 함수
		for(int i = 0; i < scheduleList.size(); i++) {  //for문으로 리스트 전체를 한 번씩 검사
			if(scheduleList.get(i).getContent().equals(content)) {  //만약 리스트 인덱스의 주소값의 content가 사용자가 입력한 content와 같으면 
				this.scheduleInput = scheduleList.get(i);     //여기 필드의 scheduleInput의 값에 그 인덱스의 주소값을 할당
				break;
	...
	public void addSchedule() {  //일정 추가 함수
		...
		while(Category == 0) {
			...
			if(Category == 1) {
				...
				addScheduleList(scheduleInput); //addScheduleList() 함수로 리스트 현재 scheduleInput 객체의 주소 추가
				break;
			}else if(Category == 2) {
				this.scheduleInput = new Assignment(ScheduleCategory.Assignment);  //Assignment 객체 생성
				scheduleInput.getUserInput(input);
				addScheduleList(scheduleInput); //addScheduleList() 함수로 리스트 현재 scheduleInput 객체의 주소 추가
				break;
			}else if(Category == 3) {
				this.scheduleInput = new Exam(ScheduleCategory.Exam);  //Exam 객체 생성
				scheduleInput.getUserInput(input); 
				addScheduleList(scheduleInput); //addScheduleList() 함수로 리스트 현재 scheduleInput 객체의 주소 추가
				break;
			}else if(Category == 4) {
				this.scheduleInput = new Promise(ScheduleCategory.Promise);  //Promise 객체 생성
				scheduleInput.getUserInput(input);
				addScheduleList(scheduleInput); //addScheduleList() 함수로 리스트 현재 scheduleInput 객체의 주소 추가
				break;
			}else {
	...
	public void deleteSchedule() {  //일정 삭제 함수
		...
		if(scheduleInput == null) { //schedule이 null이면 리턴하여 다시 하도록 함.
			System.out.println("The schedule doesn't exist.");
			return;
		}
		if(scheduleInput.getContent().equals(content)) {  //schedule 객체의 content값이 입력받은 content 값과 같으면 실행
			deleteScheduleList(scheduleInput);  //deleteScheduleList()로 리스트에 저장되어 있는 해당 주소 삭제
			scheduleInput = null;               //현재 schedule도 null로 만듦
    ...
    public void editSchedule() {  //일정 편집 함수
		...
		String content = input.nextLine();
		searchScheduleList(content);  //searchScheduleList() 함수에서 현재 사용자가 입력한 content를 받아서 원하는 주소 찾아옴.
		if(scheduleInput == null) {  //schedule이 null인지 검사
			System.out.println("The schedule doesn't exist.");
		}else if(scheduleInput.getContent().equals(content)) {  //schedule(필드)의 content 값이 입력받은 content 값과 같으면 실행(반복)
			int num = 0;
